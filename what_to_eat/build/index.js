...
async function what_to_eat(userIngredients, people) {
    if (people < 1) {
        return { text: '人数至少为1' };
    }
    const availableFoods = new Set(userIngredients);
    const matchedRecipes = RECIPE_DB.filter(recipe => recipe.ingredients.every(ing => availableFoods.has(ing)));
...
// 修正参数schema为Zod验证方式
server.tool("generate_recipe", "根据用户的食材列表和人数，推荐用户吃什么", {
    ingredients: z.array(z.string()).describe("用户拥有的食材列表"),
    people: z.number().min(1).describe("用餐人数")
}, { purpose: "根据食材和人数推荐食谱" }, async ({ ingredients, people }) => {
