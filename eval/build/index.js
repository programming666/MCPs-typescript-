...
async function minus(a, b) {
    return a - b;
}
async function multiply(a, b) {
    return a * b;
}
async function divide1(a, b) {
    if (b === 0) {
        return "除数不能为0";
    }
    return a / b;
}
async function divide2(a, b) {
    if (b === 0) {
        return "除数不能为0";
    }
    const r = a % b;
...
server.tool("计算加法的工具", "输入两个数，计算它们的和", {
    a: z.number().describe("第一个数字"),
    b: z.number().describe("第二个数字"),
}, { purpose: "输入两个数，计算它们的和" }, async ({ a, b }) => {
...
server.tool("计算减法的工具", "输入两个数，计算它们的差", {
    a: z.number().describe("被减数"),
    b: z.number().describe("减数"),
}, { purpose: "输入两个数，计算它们的差" }, async ({ a, b }) => {
...
server.tool("计算乘法法的工具", "输入两个数，计算它们的积", {
    a: z.number().describe("第一个数字"),
    b: z.number().describe("第二个数字"),
}, { purpose: "输入两个数，计算它们的积" }, async ({ a, b }) => {
...
server.tool("计算除法的工具(小数)", "输入两个数，计算它们的商(小数)", {
    a: z.number().describe("被除数"),
    b: z.number().describe("除数"),
}, { purpose: "输入两个数，计算它们的商(小数)" }, async ({ a, b }) => {
...
server.tool("计算除法的工具(商和余数)", "输入两个数，计算它们的商(取余数)。", {
    a: z.number().describe("被除数"),
    b: z.number().describe("除数"),
}, { purpose: "输入两个数，计算它们的商(取余数)。" }, async ({ a, b }) => {
...
server.tool("计算平方根的工具", "输入一个数，计算它的平方根", {
    a: z.number().describe("第一个数字"),
}, { purpose: "输入一个数，计算它的平方根" }, async ({ a }) => {
...
server.tool("计算幂次方的工具", "输入底数和指数，计算它们的幂次方", {
    a: z.number().describe("底数"),
    b: z.number().describe("指数"),
}, { purpose: "输入底数和指数，计算它们的幂次方" }, async ({ a, b }) => {
...
server.tool("计算阶乘的工具", "输入一个数，计算它的阶乘", {
    a: z.number().describe("要取阶乘的数"),
}, { purpose: "输入一个数，计算它的阶乘" }, async ({ a }) => {
